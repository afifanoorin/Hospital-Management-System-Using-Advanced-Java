 ...
 * @author Lenovo
 */
public class docChangePassword extends HttpServlet {
        ...
        try {
          ...
          out.println("<head>");
            Class.forName("net.ucanaccess.jdbc.UcanaccessDriver");//Loading Driver
              ...
              String uid= session.getAttribute("did").toString();
              String uname= session.getAttribute("dname").toString();
               long time = System.currentTimeMillis(); 
                ...
                int status=1;
                //SELECT password FROM  doctors where password='".md5($_POST['cpass'])."' && docEmail='".$_
                String sql="Update doctors set password=?, updationDate=? where id=?"; 
                ...
                if(i!=0)
                    {
                        response.sendRedirect("doctor/change-password.jsp?mess=success"); 
                     }
                else{ 
                    response.sendRedirect("doctor/change-password.jsp?mess=error"); 
                }
