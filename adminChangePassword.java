            ...
            Class.forName("net.ucanaccess.jdbc.UcanaccessDriver");//Loading Driver
              ...
              String aid= session.getAttribute("aid").toString(); 
               long time = System.currentTimeMillis(); 
                ...
                String sql="Update admin set password=?, updationDate=? where id=?"; 
                PreparedStatement ps=connection.prepareStatement(sql); 
                ps.setString(1, newpassword);  
                ps.setString(2, strDate);  
                ps.setInt(3, Integer.parseInt(aid));   
                int i=ps.executeUpdate();
                if(i!=0)
                    {
                        response.sendRedirect("admin/change-password.jsp?mess=success"); 
                     }
                else{ 
                    response.sendRedirect("admin/change-password.jsp?mess=error"); 
                }
